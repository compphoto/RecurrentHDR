...
def replace_batchnorm(module):
    """Replace all BatchNorm2d layers with GroupNorm layers."""
    if isinstance(module, nn.BatchNorm2d):
        num_features = module.num_features
        eps = module.eps
        affine = module.affine
        new_module = nn.GroupNorm(
            num_groups=1, num_channels=num_features, eps=eps, affine=affine
        )
        return new_module
    for name, child in module.named_children():
        new_child = replace_batchnorm(child)
...
def _make_encoder(
    ...
    if backbone == "resnext101_wsl":
        ...
        pretrained = _make_pretrained_efficientnet_lite3(
            use_pretrained, exportable=exportable, in_chan=in_chan
        )
        scratch = _make_scratch(
            [32, 48, 136, 384], features, groups=groups, expand=expand
        )  # efficientnet_lite3
    elif backbone == "efficientnet_b5":
        pretrained = _make_pretrained_efficientnet_b5(
            use_pretrained, exportable=exportable, in_chan=in_chan
        )
        scratch = _make_scratch(
            [40, 64, 176, 512], features, groups=groups, expand=expand
        )  # efficientnet_lite3
    elif backbone == "efficientnet_b6":
        pretrained = _make_pretrained_efficientnet_b6(
            use_pretrained, exportable=exportable, in_chan=in_chan
        )
        scratch = _make_scratch(
            [40, 72, 200, 576], features, groups=groups, expand=expand
        )  # efficientnet_lite3
    else:
        print(f"Backbone '{backbone}' not implemented")
        assert False
...
def _make_pretrained_efficientnet_b5(use_pretrained, exportable=False, in_chan=3):
    efficientnet = torch.hub.load(
        "rwightman/gen-efficientnet-pytorch",
        "tf_efficientnet_b5",
        pretrained=use_pretrained,
    ...
    if in_chan != 3:
        efficientnet.conv_stem = Conv2dSame(
            in_chan, 48, kernel_size=(3, 3), stride=(2, 2), bias=False
        )

    return _make_efficientnet_backbone(efficientnet)


def _make_pretrained_efficientnet_b6(use_pretrained, exportable=False, in_chan=3):
    efficientnet = torch.hub.load(
        "rwightman/gen-efficientnet-pytorch",
        "tf_efficientnet_b6",
        pretrained=use_pretrained,
    ...
    if in_chan != 3:
        efficientnet.conv_stem = Conv2dSame(
            in_chan, 56, kernel_size=(3, 3), stride=(2, 2), bias=False
        )
...
class FeatureFusionBlock(nn.Module):
    ...
    def __init__(self, features, scale=2):
        """Init.
